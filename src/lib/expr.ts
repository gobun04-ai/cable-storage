...
/**
 * 합계와 함께 수식 원문도 보여 줄지 판단한다.
 *
 * "45" 처럼 적은 값이 곧 결과인 식은 두 번 적을 이유가 없다.
 * 그 밖에는 어떤 연산이 들어갔든 원문을 남긴다 — 12*3 은 "12 m 짜리 세 가닥"이라는
 * 뜻이고, 36 만 남으면 나중에 어떻게 나온 수인지 되짚을 수 없다.
 *
 * 화면과 공유 텍스트가 같은 규칙을 쓰도록 판단을 여기 한 곳에 둔다.
 * 두 곳에 따로 두었더니 곱셈이 든 식이 화면에는 보이고 공유 텍스트에서는 빠졌다.
 */
export function shouldShowExpression(expression: string, value: number): boolean {
  return expression.trim() !== String(value)
}
